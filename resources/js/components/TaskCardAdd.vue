<template>
    ...
    <v-menu v-model="menu">
        ...
        <v-date-picker v-model="cardForm.limit" @input="formatDate(cardForm.limit)"></v-date-picker>
    </v-menu>
...
<script>
  ...
  export default {
    ...
    computed:{
        contentExists() {
        return this.cardForm.name.length > 0 && this.cardForm.content.length > 0 && this.cardForm.limit.length > 0 && this.cardForm.status.length > 0
      },
    },
    methods:{
        async addCardToList(){
